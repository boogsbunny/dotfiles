...
(defun boogs/sly-start-new-connection ()
  "Start a brand-new SLY connection, prompting for the Lisp
implementation."
  (interactive)
  ;; Negative prefix argument (C--), i.e. current-prefix-arg = '-,
  ;; forces SLY to open a *new* connection.
      ...
      (let* ((maybe-core-dumper (when-let ((exec (executable-find "lisp-repl-core-dumper")))
        ...
        `((sbcl-boogs ("lisp-repl-core-dumper" "-g" "--non-interactive"
          ...
          (sbcl-nyxt-site ("guix" "environment" "--pure"
                           ...
                           "--" "sbcl"))
          (abcl (,@maybe-core-dumper "abcl"))
          (ccl (,@maybe-core-dumper "ccl"))
          (clisp (,@maybe-core-dumper "clisp"))
          (ecl (,@maybe-core-dumper "ecl")))))

(setq sly-connection-update-interval 0.1)

(when (require 'helm nil :noerror)
  (with-eval-after-load 'sly-mrepl
    (define-key sly-mrepl-mode-map (kbd "C-f") 'helm-comint-input-ring)
...
(with-eval-after-load 'sly-mrepl
  (define-key sly-mrepl-mode-map (kbd "C-f") 'consult-comint-input-ring))
...
(with-eval-after-load 'sly-mrepl
  ...
  (define-key sly-mrepl-mode-map (kbd "C-c d") 'common-lisp-hyperspec)
